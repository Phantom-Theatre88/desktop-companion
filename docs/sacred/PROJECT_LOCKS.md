...
現在の次工程では、外部感覚器官を横方向に増やすことよりも、すでに実機で通っている Camera / Touch / IMU を使い、

**Semantic Neuron → Ghost → Heart → Behavior → Face / Neck**

までを縦方向に生命として成立させることを優先する。

次の優先順は、

1. Heartをイベント・時間で実際に変化させる
2. Ghost状態から理由のある自発Behaviorを成立させる
3. M5Stackちゃん単独で「そこにいる感じ」を成立させる
4. その後にToF4M等の外部感覚を追加する

とする。

ToF4Mが「最初の外部感覚」であるLOCKは維持するが、「現在の次工程」であるとは限らない。

LOCK 54の A→B→C→D→E は維持する。ただしAを「すべての感覚器官を実装し終えてからBへ進む工程」と解釈せず、現在ある感覚を使って **感じる → 気分が変わる → 自分から行動する** を一本の縦貫通として先に成立させる。

詳細は `LOCK55_M5単独生命成立を優先する縦貫通実装.md` を正とする。


## LOCK 56｜まぶた／目表現の外部参照候補

以下の動画を、Desktop Companionの**まぶた／目表現の実装候補を検討するための正式参照候補**として登録する。

- YouTube: https://www.youtube.com/watch?v=pQeTcOIVzHc

このLOCKは、動画内の具体的な実装方式・数値・アニメーション手法をそのまま採用することを意味しない。

現時点では動画内容をこちらで直接確認できていないため、**参照候補としてのみLOCK**する。具体方式の採否は、内容確認後に現行の独自 EyeShape / FaceRenderer / BehaviorEngine 構造へ適合するかを評価して決める。

特に検討対象は、現在の水平まぶたマスクだけでなく、上まぶたの傾き（/・\\）、左右差、開眼量、視線・目形状との組み合わせによる方向・注意・感情表現とする。

既存OSSや外部表現へ本体設計を合わせず、**構造差分確認 → 適合設計 → 再実装 → 実機確認**を原則とする。


## LOCK 57｜口・一時オブジェクト表現

Desktop Companionの顔は引き続き**大きな2眼を主役**とし、口を常設の主表現にはしない。

ただし、口を禁止はしない。必要なイベント・行動時にのみ現れる**一時的なBehavior演出部品**として採用候補に含める。

想定例：

- SHAKE後に「うっ…」となり、必要なら一時的に「ゲロを吐く」ようなコミカル表現を出す
- boredom / hunger相当の状態や自発Behaviorで、何かを「咥える」ような小物表現を出す
- 驚き時に小さな `o` 口を出す
- 発話時のみ口を表示・変形する
- あくび等、目だけでは伝わりにくい行動表現を補助する

設計上の位置づけは、**目＝内面・注意・感情の主表現、口／小物＝Behavior結果を補強する副表現**とする。

口・小物はFaceRendererへ常時固定するのではなく、Heart / Reflex / Behaviorから必要時のみ出力される一時表現として扱う。

具体的な形状、アニメーション、出現条件、持続時間は未LOCKとし、表情語彙全体の実機評価後に決める。

## LOCK 58｜アニメ記号・小物による補助表現語彙

Desktop Companionの身体表現は、引き続き**大きな2眼を内面・注意・感情の主表現**とする。

その上で、顔文字・漫画・アニメで一般的に使われる視覚記号を参考にし、目だけでは伝わりにくい一時的な状態・行動を補助する**アニメ記号／Visual Effect／小物表現**を正式な表現語彙として拡張していく。

表現構造は、

**Heart / Context / Behavior → Face + Visual Effect + Prop**

とし、完成表情プリセットを切り替える方式にはしない。

初期の正式候補語彙は以下とする。

- **涙**：悲しさ、感動、つらさ等を補助する。目尻からの一滴、連続涙等を状況で変えられる
- **汗**：焦り、困惑、緊張、冷や汗等を補助する。意味に応じて大きさ・位置・速度を変えられる
- **照れ**：頬の斜線、淡い面表現等
- **疑問記号 `?`**：理解できない、気になる、迷う等
- **気づき記号 `!`**：発見、注意、突然の認識等
- **キラキラ／星**：期待、興奮、嬉しさ、強い興味等
- **怒りマーク**：苛立ち、強い不快等の補助
- **睡眠記号 `Z / z`**：熟睡状態の補助。固定文字列ではなく、浮上・縮小・消失等のアニメーションを基本とする
- **一時小物**：コーヒーカップ等。Behaviorとして必要な時だけ出し、口・目・首の動きと組み合わせて短い演技として成立させる

同じ記号でも意味を固定しない。

たとえば `SWEAT` は、怖さ、焦り、困惑等の異なるHeart / Contextから使われてよく、**何を意味するかはHeart / Context / Behavior側が決める**。

Visual Effectや小物は、目と同じ色・同じ形状言語へ溶け込ませず、**本人の顔・状態記号・小物が視覚的に区別できる表現**とする。

単なる静止アイコン表示ではなく、可能な範囲で、
...
を使い、**短いアニメーション演技**として見えることを目標とする。

M5GFXで軽量に描ける図形・文字・簡易ベクター表現を基本とし、重い画像アセットの常時展開を前提にしない。
