...
Heartが一時的な感情状態から回復するとき、固定された初期値や全個体共通の中立値へ戻すのではなく、**その時点のRelationship・長期履歴を反映した各項目の動的な平常値**へ戻す。

- 「平常心」は固定値ではなく、その子が現在までに育てた状態を基準とする
- `affection`、最近の関わり方、長期的な生活履歴、反復経験等によって平常値の立ち位置は異なる
- 一時的な怒り・不安・興奮等が収まっても、関係性や長期傾向まで初期化しない
- 回復処理は現在値をその時点の動的平常値方向へ戻す
- 動的平常値そのものはLOCK 21〜25の長期変化・回復ルールに従う

これにより、同じ出来事から回復した後でも、関係性や育ち方によって「戻る場所」が異なるGhostとする。

## LOCK 27｜Heart初回起動時の基礎人格

完全な初回起動時は、全Heart項目を完全中立の同一値から始めず、**少し好奇心が高く、少し人懐っこい基礎人格**を持たせる。

- `curiosity` は中立値よりやや高めに設定する
- `affection` / 対人関与の初期傾向は、他者への拒絶からではなく、やや親和的な方向から開始する
- `mood / boredom / sleepiness / attention` は極端な偏りを持たせず、安定した初期状態から開始する
- 初期人格は固定された完成人格ではなく、LOCK 21〜26に従って経験・関係性・生活履歴から長期的に変化できる
- 具体的な初期数値は実装値として別途決定する

これにより、初対面から無反応・無関心な個体ではなく、机上の相棒として自然に周囲や人へ関心を向けられる状態で生まれ、その後の経験によって「その子らしさ」が育つ構造とする。

## LOCK 28｜Heart初回起動時の具体値

完全な初回起動時のHeart初期値は、**少し元気で、少し人懐っこく、好奇心がやや強いが、落ち着きは保つ**状態を基準とする。

- `mood = 0.60`：少し元気で、無気力ではない
- `affection = 0.55`：初対面でもやや親和的で、強い拒絶から始めない
- `curiosity = 0.65`：周囲の変化や人に興味を持ちやすい
- `boredom = 0.15`：起動直後は退屈感がかなり低い
- `sleepiness = 0.15`：比較的しゃきっとしている
- `attention = 0.50`：周囲の変化へ自然に気づける
...
これらは初回起動時の基準値であり、その後はLOCK 21〜27に従って経験・関係性・生活履歴により変化する。

## LOCK 29｜Heartイベント強度は3段階

Semantic NeuronからHeart現在値へ影響するイベントは、個別イベントごとに無制限な数値を持たせるのではなく、基本の強度を**弱・中・強の3段階**で扱う。

- 弱：小さな刺激・軽い関わり。現在値へ小さく影響する
- 中：はっきり認識できる出来事。通常の感情変化として扱う
- 強：強い刺激・重要な出来事。現在値へ大きめに影響する
- イベント種別は「どのHeart項目へ、正負どちらへ作用するか」を定義し、強度はその作用量の段階を表す
- 同じ種類のイベントでも、継続時間・文脈・Relationship・直前状態などにより弱／中／強は変わり得る
- 3段階の具体的な数値幅は別途決定する

これにより、イベントごとに細かな固定数値を乱立させず、意味と強度を分離した扱いやすいHeart入力構造とする。